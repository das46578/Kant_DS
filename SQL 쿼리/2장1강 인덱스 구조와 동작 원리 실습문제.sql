...
3. 인덱스 생성 전 실행계획에서 다음 항목을 기록하세요.
   - 스캔 방식: Seq Scan
   - cost: 0.00..6321.17 / 1000.00..7321.77
   - actual rows: 2 / 6
   - Execution Time: 32.694
...
8. 다음 질문에 답하세요.
   ...
   Q2. B-Tree 인덱스가 customer_id = 31428을 찾을 때
   	...
   	- 따라서 1번부터 2...3...번 순서로 스캔하지 않고 테이블 행 모두를 한번에 확인하여 검색 범위를 좁혀서 속도가 빠름.
...
--1번
drop index if exists idx_orders_cutomer_id;
...
8. 다음 질문에 답하세요.
   Q1. CREATE INDEX와 DROP INDEX는 각각 어떤 작업을 수행하나요?
   	- Create Index: 지정한 컬럼 값을 기반으로 인덱스 구조를 생성
...
from pg_indexes
where schemaname = 'pm2' and tablename = 'products'
order by  indexname;
...
8. 인덱스 생성 전후 결과를 비교하세요.
   - 스캔 방식이 Seq Scan에서 Index Scan으로 변경
   - cost와 Execution Time이 감소
   - actual rows는 동일하므로 같은 결과를 더 효율적으로 조회한 것을 알 수 있음.
9. 실습이 끝나면 idx_products_price 인덱스를 삭제하세요.
10. 다음 질문에 답하세요.
    Q1. B-Tree 인덱스는 등호 검색 외에 어떤 비교 조건에 활용될 수 있나요?
     - <, >, BETWEEN과 같은 비교와 범위 검색에 활용됨.
     
    Q2. B-Tree 인덱스가 범위 검색에서 검색 범위를 줄일 수 있는 이유는 무엇인가요?
     - 전체 데이터를 하나씩 확인하지 않고 값의 범위를 단계적으로 좁혀 탐색하기 때문
       
    Q3. 인덱스를 많이 만들수록 INSERT, UPDATE, DELETE 비용이 커질 수 있는 이유는 무엇인가요?
	 - 테이블에 인덱스가 많을수록 쓰기 작업 하나당 갱신해야 할 인덱스 개수도 늘어나기 때문
...
1. B-Tree 인덱스가 검색 속도를 높일 수 있는 핵심 원리는 무엇인가요?
  - B-Tree는 키를 정렬된 트리 구조로 관리하여 전체 데이터를 순서대로 읽지 않고 검색 범위를 줄일 수 있다.
...
3. 인덱스를 만들 때 조회 성능뿐 아니라 쓰기 성능도 고려해야 하는 이유는 무엇인가요?
  - INSERT, UPDATE, DELETE 시 테이블뿐 아니라 관련 인덱스의 항목 추가 및 유지 그리고 과거 항목 정리가 필요할 수 있음.
