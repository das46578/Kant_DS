...
8. 데이터 분포만을 기준으로 보았을 때
   ...
   - customer_id > 카디널리티가 높고, 선택도가 낮기 때문
...
4. 두 실행계획에서 다음 항목을 비교하세요.
   ...
   - Execution Time
항목				  |	조건 A		|	조건 B
스캔 방식			  |	Index Scan	|	Seq Scan
조건 표시 위치		  |	Index Cond	|	Filter
cost			  |	0.00..4.48	|	0.00..4558.06
actual rows/ loop | 19/1		|	19 / 1
Execution time	  | 0.088 ms	|	92.880 ms

5. 다음 질문에 답하세요.
   Q1. 두 쿼리는 같은 customer_id를 찾는데도 실행계획이 달라질 수 있는 이유는 무엇인가요?
   	- 일반 B-Tree 인덱스는 cutomer_id의 원래 값을 기준으로 구성되어 있음.
   	- cutomer_id = 31428은 인덱스의 키를 직접 비교
   	- cutomer_id + 0 =31428은 컬럼을 가공한 결과를 기준으로 비교
...
Execution Time	|	0.029 ms	|	0.051 ms

9. 다음 네 가지 기준으로 stores.city 인덱스의 적절성을 판단하세요.
   - 카디널리티: city 컬럼의 고유값 수는 4개로 전체 100행에 비해 카디널리티가 낮기 때문에 인덱스 효과가 크지 않을 수 있음.
   - 선택도: city 컬럼은 4개의 값이 반복 저장되어 있기 때문에 선택도가 높아 인덱스 효과가 크지 않을 수 있음.
   - 테이블 크기: stores 테이블은 전체 100행으로 크기가 크지 않기 때문에 데이터 전체를 읽는 비용이 높지 않을 수도 있음.
   			   따라서 Seq Scan이 더 효율적일 수도 있음.
   - 인덱스 유지 비용: INSERT, UPDATE, DELETE 시 인덱스도 함께 갱신해야 하므로 추가 비용이 발생함.
10. 최종적으로 "인덱스 추가를 적극적으로 권장한다 / 우선순위가 낮다" 중 하나를 선택하고 근거를 작성하세요.
   - 우선순위가 낮다. 인덱스 생성 전후 모두 Seq Scan이 사용되었고 Execution Time도 0.029 ms에서 0.051 ms로 성능 향상이 확인되지 않음.
...
1. 카디널리티가 높고 선택도가 낮은 조건에서 인덱스가 유리한 이유는 무엇인가요?
  - 서로 다른 값이 많고 조건을 만족하는 행이 적으면
    인덱스를 통해 전체 테이블을 읽지 않고 필요한 소수의 행 위치만 빠르게 찾을 수 있음

2. 인덱스가 존재해도 PostgreSQL이 Seq Scan을 선택할 수 있는 이유는 무엇인가요?
  - 옵티마이저는 인덱스 존재 여부만 보는 것이 아니라
    조회해야 할 행의 비율, 테이블 크기, 통계 정보 등을 이용해 예쌍 비율을 비교
  - 많은 행을 읽어야 하거나 테이블이 작은 편ㅇ이라면 Seq Scan의 비용이 더 낮을 수도 있음. 
...
4. 인덱스 추가 여부를 판단할 때 확인해야 할 네 가지 기준은 무엇인가요?
  ...
  - 조건을 적용했을 때 선택도가 낮은가?
  - 테이블 규모가 충분히 큰가?
  - 조회 이득이 INSERT, UPDATE, DELETE 시 발생하는 인덱스 유지 비용을 감당할 만큼 큰가?
