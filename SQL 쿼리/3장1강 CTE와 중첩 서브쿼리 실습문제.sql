...
/*
Parallel Seq Scan: 여러 작업자가 'orders', 'order_items' 데이터를 나누어 읽는다.
Parallel Hash: 주문번호로 빠르게 연결할 수 있도록 'orders'의 해시 자료 구조를 만든다.
Parallel Hash Join: 같은 'order_id'를 가진 주문과 주문 상세를 연결한다.
 */
...
11. 다음 질문에 답하세요.
   Q1. 중첩 서브쿼리와 CTE의 결과는 달라야 하나요?
   	- ㄴㄴ 두 쿼리가 같은 테이블, JOIN 등 조건이 같다면 결과는 동일해야 함.
   	
   Q2. 여러 단계의 로직을 읽을 때 CTE가 더 이해하기 쉬울 수 있는 이유는 무엇인가요?
   	- 각 중간 처리 결과에 의미 있는 이름을 붙일 수 있음.
   	- ex) Customer_totals
   	
   Q3. 이번 문제처럼 중간 결과에 이름을 붙이는 것이 유지보수에 어떤 도움이 되나요?
   	- 나중에 쿼리를 수정하거나 다른 개발자가 코드를 확인할 때 각 단계가 어떤 역할을 수행했는지 빠르게 파악 가능
...
6. 다음 질문에 답하세요.
   Q1. 일반 CTE는 항상 CTE Scan으로 나타나나요?
   	- ㄴㄴ PostgreSQL Ver.12 이상에서는 조건에 따라 CTE를 본문 쿼리에 인라인 처리 가능
   	- CTE Scan이 나타나지 않을 수 있음.
   	
   Q2. MATERIALIZED를 사용하면 처리 흐름이 어떻게 달라지나요?
   	- CTE를 본문과 합치지 않고 별도의 결과로 다룸.
   	- 본문은 그 결과를 CTE Scan으로 참조하는 구조
   	
   Q3. MATERIALIZED가 항상 더 빠르다고 말할 수 있나요?
	- ㄴㄴ 중간 결과를 ㅂㄹ도로 계산하고 저장한 뒤에 다시 읽는 비용이 발생할 수 있음
...
8. 다음 질문에 답하세요.
   Q1. 이 문제를 하나의 중첩 서브쿼리로 작성하는 것보다 CTE로 나누었을 때 어떤 장점이 있나요?
   	- CTE로 나누면 복잡한 로직을 단계별로 분리할 수 있어 쿼리의 흐름을 쉽게 파악할 수 있음.
   	
   Q2. customer_totals와 high_value_customers라는 이름은 각각 어떤 처리 단계를 의미하나요?
  	- customer_totals: 고객별 총 구매금액을 계산하는 단계
  	- high_value_customers: 그중 구매금액이 50000 이상인 고객을 선별하는 단계
  
   Q3. 성능 차이가 거의 없다면 CTE와 중첩 서브쿼리 중 어떤 기준으로 구조를 선택하는 것이 좋나요?
   	- 가독성과 유지보수성을 기준으로 구조를 선택하는 것이 좋으나
   	  실행계획상 성능 차이가 크지 않은 상황이라면 로직을 명확한 이름으로 분리할 수 있는 CTE가 좋음.
   
   Q4. 이번 EXPLAIN 결과를 기준으로 CTE가 실제 실행 단계에서
       반드시 별도의 중간 결과로 저장되었다고 말할 수 있나요?
       실행계획을 근거로 설명하세요.
   	- CTE Scan이 나타나지 않고 CTE의 연산과 필터가 본문 실행계획에 합쳐져 있으므로
   	  별도의 중간 결과로 저장된 것이 아닌 인라인되어 최적화됐다고 할 수 있음.
