...
8. 다음 질문에 답하세요.
   ...
   Q2. 같은 order_id가 여러 행으로 나타났다고 해서 무조건 잘못된 중복인가요?
   	- ㄴㄴ 한 주문에 여러 상품이 포함되어 있으면 ORDER_ID가 여러 행으로 나타날 수 있음.
...
8. 다음 질문에 답하세요.
   Q1. WHERE에 o.store_id = 1을 작성하면 주문이 없는 고객이 왜 사라지나요?
   	...
   	- 그 후 where o.store_id = 1 조건을 적용하면 
   	  null = 1인 행은 True가 아니라 unkown으로 True행만 남기는 where에서 해당 고객이 제외
   	
   Q2. ON에 o.store_id = 1을 작성하면 주문이 없는 고객이 왜 유지되나요?
   	- on 조건은 어떤 orders행을 customers와 연결할지를 결정
   	- 1번 매장 주분이 ㅇ벗더라도 left join의 왼쪽 테이블인 customer행은 유지
   	- order 컬럼만 null로 표시
   	
   Q3. "전체 고객을 유지하면서 1번 매장 주문만 연결"이라는 요구사항에는 어느 쿼리가 더 적절한가요?
   	- on절에 o.store_id = 1 조건 작성한 쿼리가 적절
...
1. 다단계 조인을 작성할 때 가장 먼저 확인해야 하는 것은 무엇인가요?
  - 최종 결과에 필요한 컬럼이 어느 테이블에 있는지 테이블간 연결키 및 관계 유형 확인 필요
  
2. 1:N 관계에서 행 수가 증가하는 이유는 무엇인가요?
  - 기준 테이블의 한 행에 상대 테이블의 ㅇ러 행이 연결될 수 있음
  - 기준 행이 매칭되는 개수만큼 여러 행으로 확정되기 때문

3. LEFT JOIN에서 오른쪽 테이블 조건을 ON과 WHERE에 둘 때 결과가 달라지는 이유는 무엇인가요?
  - on은 어떤 행을 연결할지를 정하는데
  - where은 ㅈ조인이 끝난 결과에서 어떤 행을 남길지를 결정하기 때문
