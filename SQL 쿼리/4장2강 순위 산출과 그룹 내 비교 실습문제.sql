...
select
	...
	rank() over(order by total_qty desc) as rank_num,       --공동 2위개 2개면 1, 2, 2, 4
	dense_rank() over(order by total_qty desc) as dense_num --공동 2위개 2개여도 1, 2, 2, 3
from product_sales
order by total_qty, product_id asc;
...
9. 다음 질문에 답하세요.
   Q1. PARTITION BY category_id를 사용하면 순위 계산 범위가 어떻게 달라지나요?
   	- 전체 상품을 하나의 순위로 계산하지 않고 각 category_id별로 별도의 순위 그룹을 만든다.
   	- 왜냐면~~ 카테고리가 바뀔 때마다 순위가 다시 1부터 시작할 수 있게 하기 위해서!!
   	
   Q2. category_rank <= 3 조건을 같은 SELECT의 WHERE 절에 바로 작성할 수 없는 이유는 무엇인가요?
   	- where절이 실행되는 시점에서는 윈도우 함수로 계산된 category_rank가 아직 만들어지기 전이기 때문에
   	  CTE나 서브쿼리에서 순위를 계산한 뒤 바깥 쿼리의 where에서 필터링 한다.
   
   Q3. RANK를 사용했기 때문에 공동 3위가 여러 상품이면 결과가 3개를 초과할 수 있나요?
   	- ㅇㅇ rank는 동점에 같은 순위를 부여하기 때문에 공동 3등 상품이 여러 개면 모두 category_rank <= 3 조건을 만족
...
8. 다음 질문에 답하세요.
   Q1. NTILE(4)는 금액 범위를 정확히 4등분하나요,
   	...
   	- 정렬된 고객 수를 기준으로 최대한 균등하게 4개 그룹으로 나눔.
   	
   Q2. customer_group = 1은 어떤 고객군으로 해석할 수 있나요?
   	- total_amount desc로 정렬했기 때문에
   	  총 구매 금액이 높은 상위 고객군으로 해석할 수 있음.
   	
   Q3. 이 결과를 마케팅 업무에 어떻게 활용할 수 있나요?
   	- 구매 금액이 높은 고객군을 구분해
   	  VIP 고객 대상 프로모션이나 우선 마케팅 타깃 선정에 활용할 수 있음.
...
select
	customer_id, total_amount, customer_group,
	count(*) over(partition by customer_group),
	avg(total_amount) over(
		partition by customer_group) as avg_total_amount
...
1. ROW_NUMBER, RANK, DENSE_RANK의 가장 큰 차이는 무엇인가요?
  - ROW_NUMBER(): 동점이어도 서로 다른 번호 부여
  - RANK(): 동점은 같은 순위, 다음 순위는 건너뜀.
  - DENSE_RANK(): 동점은 같은 순위, 다음 순위는 건너뛰지 않음.
  
2. PARTITION BY를 순위 함수와 함께 사용하면 무엇이 달라지나요?
  - 전체 데이터에 한 번만 순위를 매기는 것이 아니라 그룹마다 순위를 다시 계산함.
  
3. 그룹별 상위 N개를 추출할 때 서브쿼리나 CTE가 필요한 이유는 무엇인가요?
  - 먼저 윈도우 함수로 순위를 계산한 뒤 그 결과에 WHERE rank <= N 조건을 적용해야 하기 때문
  
4. NTILE 결과를 비즈니스 지표로 해석할 때 주의해야 할 점은 무엇인가요?
  - 정렬된 행의 개수를 기준으로 최대한 균등하게 그룹을 나누기 때문에
    각 그룹의 금액 범위가 동일하다고 해석하면 안 됨.
*/
