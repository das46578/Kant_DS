...
9. 다음 질문에 답하세요.
   Q1. running_total은 어떤 범위의 매출을 합산한 값인가요?
   	- 첫 번째 날부터 현재 행까지의 daily_sales를 합산
   	
   Q2. moving_avg_7d는 현재 행을 포함해 최대 몇 개 행을 평균내나요?
   	- 7개
   	-6 preceding: 현재 행보다 앞의 6개 행을 뜻하고 current row까지 포함하여 총 7개
...
select
	order_date, daily_sales, prev_day_sales, next_day_sales,
...
10. 다음 질문에 답하세요.
    ...
    Q2. NULLIF(prev_day_sales, 0)를 사용하는 이유는 무엇인가요?
     - 분모가 0이 되는 경우를 방지하기 위해? 0으로 나누는 오류를 방지하기 위해? 사용
     
    Q3. day_over_day_pct가 음수라는 것은 비즈니스적으로 무엇을 의미하나요?
     - 현재 매출이 전일보다 감수했다는 의미
     
    Q4. 하루의 감소만으로 매출 추세가 악화되었다고 단정하기 어려운 이유는 무엇인가요?
     - 하루의 매출 감소는 일시적인 변동일 수 있으므로 여러 날짜의 흐름을 함께 확인해야 전체 추세를 판단할 수 있음.
...
sales_result as(
	select
		order_date, daily_sales, running_total, prev_day_sales,
		daily_sales - prev_day_sales as day_over_day_diff,
		round((daily_sales - prev_day_sales) * 100
			/ nullif(prev_day_sales, 0)
...
1. 누적합과 이동평균의 계산 범위는 어떻게 다른가요?
  ...
  - 이동평균은 현재 행을 기준으로 치젖ㅇ한 최근 N개의 행을 계산
...
4. 윈도우 함수 결과를 비즈니스 지표로 해석할 때 무엇을 주의해야 하나요?
  - 하루의 증감이나 하나의 지표만으로 전체 추세를 단정해서는 안 됨.
