...
10. 다음 질문에 답하세요.
    ...
    Q3. WindowAgg 이전에 Sort가 나타날 수 있는 이유는 무엇인가요?
      - RANK()가 PARTITION BY customer_id, ORDER BY 부분이 있기 때문에
...
select
	o.customer_id, o.order_id, oi.product_id, oi.price,
...
5. 가장 비용 또는 실제 시간이 크게 나타나는 노드를 병목 후보로 정하세요.
   	...
   	- 2. 처리 행 수가 많은 노드
   	- 3. Row Removed by Fiter가 많은 노드
   	- 4. Sort과 같이 추가 비용이 발생하는 노드
6. 다음 질문에 답하세요.
   Q1. cost와 actual time은 어떤 차이가 있나요?
   	- cost는 옵티마이저가 계싼한 상대적 비용
...
6. 실행계획에서 다음 항목을 확인하세요.
   ...
   - estimated rows: 74478
   - actual rows: 75023.00
   - Execution Time:: 125.653 ms
7. 실행계획을 아래에서 위로 읽으면서 실제 처리 흐름을 설명하세요.
   - customers Seq Scan → Hash 생성
   - orders Seq Scan → 2023년 주문만 Filter
   - Hash Join → customer_id 기준으로 두 테이블 결합
   - Sort → order_date DESC 정렬
   - 최종 결과 반환
8. 가장 먼저 확인할 병목 후보 노드를 하나 선택하고 이유를 작성하세요.
   - orders의 Seq Scan
   - 전체 행을 읽은 뒤 2023년 조건에 맞지 않는 행을 Filter로 제거하고 있기 때문
   
9. 다음 질문에 답하세요.
   Q1. Scan 노드에서는 무엇을 확인해야 하나요?
   	- 테이블을 어떤 방식으로 읽고 있는지 확인
   	
   Q2. Join 노드에서는 무엇을 확인해야 하나요?
   	- 두 테이블을 어떤 방식으로 연결하는지 확인
   	
   Q3. Sort 노드에서는 무엇을 확인해야 하나요?
   	- 어떤 기준으로 정렬하는지와 정렬 비용이 크게 발생하는지 확인
   	
   Q4. Seq Scan이 나타났다고 해서 무조건 잘못된 실행계획이라고 할 수 있나요?
   	- ㄴㄴ 데이터 양이나 조회 조건에 따라 옵티마이저가 Seq Scan이 더 효율적이라고 판단할 수 있기 때문
...
1. 실행계획은 왜 아래에서 위로 읽는 것이 좋은가요?
  - 실행계획은 하위 노드의 결과가 상위 노드로 전달되는 구조
...
4. 병목 노드를 찾을 때 어떤 정보를 함께 봐야 하나요?
  - cost, actual time, rows, Row Removed by Filter 등
  - 1. actual time이 상대적으로 큰 노드
  - 2. 처리 행 수가 많은 노드
  - 3. Row Removed by Fiter가 많은 노드
  - 4. Sort롸 같이 추가 비용이 발생하는 노드
*/
