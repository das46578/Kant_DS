...
9. 다음 질문에 답하세요.
   ...
   Q3. 인덱스를 생성했는데 Execution Time이 반드시 감소한다고 단정할 수 있나요?
	- ㄴㄴ 데이터의 상태, 메모리, 서버 등에 따라 영향을 받을 수 있음
...
--개선 후 동일 쿼이 재측정
explain analyze
select order_itme_id, order_id, procudt_id, qty, price
from order_items
where product_id = 100;
...
6. 인덱스 생성 전 EXPLAIN ANALYZE 결과에서 다음 항목을 기록하세요.
   ...
   - Join 방식: Hash Join
   - JOIN 조건: o.customer_id = c.customer_id
   - Sort 여부: o.order_date DESC
   - total cost: 7503.76
   - actual rows: 6344
   - Execution Time: 35.212 ms
7. orders.order_date에 idx_orders_order_date 인덱스를 생성하세요.
8. 동일한 JOIN 쿼리에 다시 EXPLAIN ANALYZE를 적용하세요.
9. 개선 후 동일한 항목을 기록하세요.
   - orders 스캔 방식: Bitmap Heap Scan + Bitmap Index Scan
   - customers 스캔 방식: Seq Scan
   - Join 방식: Hash Join
   - JOIN 조건: c.customer_id = o.customer_id
   - Sort 여부: 있음 → o.order_date DESC
   - total cost: 3446.80
   - actual rows: 6344
   - Execution Time: 8.600 ms
10. Join 노드가 개선 전후에 어떻게 달라졌는지 확인하세요.
    - Join 방식이 변경되었는지: Hash Join → Hash Join
    - Join 입력 행 수가 달라졌는지: ㄴㄴ
    - Join 노드의 cost 또는 actual time이 달라졌는지
     -> cost는 6270.20 → 3363.92로 감소
        actual time은 8.840 ms → 7.483 ms로 감소
11. 실행시간 감소율을 다음 식으로 계산하세요.

   (개선 전 Execution Time - 개선 후 Execution Time)
   / 개선 전 Execution Time * 100

	(35.212 - 8.600) / 35.212 * 100
	= 75.58%
12. 다음 형식으로 비교 결과를 정리하세요.
   ...
   JOIN 조건			|	o.customer_id = c.customer_id	|	c.customer_id = o.customer_id
   Sort 여부			|	order_date DESC					|	order_date DESC
   Join 노드 변화		|	Hash Join						|	Hash Join 유지, cost와 actual time 감소
   total cost		|	7503.76							|	3446.80
   actual rows		|	6344							|	6344
   Execution Time	|	35.212 ms						|	8.600 ms
   실행시간 감소율		|	75.58% 감소

13. 다음 질문에 답하세요.
    Q1. 인덱스 추가 후에도 Sort가 남을 수 있나요?
     - ㅇㅇ 인덱스를 추가하더라도 정렬 작업이 필요하면 Sort가 남을 수 있음.
     
    Q2. 인덱스를 추가했는데 옵티마이저가 Seq Scan을 계속 선택한다면 어떤 의미인가요?
     - 현재 데이터와 조회 조건에서는 옵티마이저가 Seq Scan이 더 효율적이라고 판단했다는 뜻
     
    Q3. Join 방식이 변경되었다고 해서 반드시 성능이 개선되었다고 말할 수 있나요?
     - ㄴㄴ 성능 개선 여부는 Join 방식만 보는 것이 아니라
       Execution Time과 같은 실제 실행 시간도 함께 비교해야 함.
       
    Q4. 이번 결과만으로 모든 날짜 JOIN 조회에 order_date 인덱스가 항상 효과적이라고 결론 내릴 수 있나요?
	 - ㄴㄴ 인덱스가 있어도 데이터 양과 조회 조건에 따라 다른 실행 계획이 선택될 수 있음.
...
1. 튜닝 전에 기준 실행계획을 먼저 기록해야 하는 이유는 무엇인가요?
  - 개선 전 스캔, cost, rows, actual_time 등 실제 변경 전후를 비교하기 위해
  
2. 인덱스 추가와 쿼리 재작성 후 무엇을 다시 측정해야 하나요?
  - 동일한 쿼리 조건으로 EXPLAIN ANALYZE를 다시 실행
  - cost, scam actual rows 등의 변화 확인
...
4. 실행계획이 바뀌었다는 사실만으로 성능 개선을 확정할 수 없는 이유는 무엇인가요?
  - 실행 방식을 튜닝해도 데이터 분포와 서버 상태에 따라 성능이 방드시 개성되었다고 보긴 어려움. 
